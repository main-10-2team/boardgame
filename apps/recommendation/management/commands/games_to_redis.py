...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        try:
            ...
            full_vector_dimension = len(all_genres) + len(all_categories) + scaled_numerical_features.shape[1]
            today_vector_dimension = len(all_categories) + scaled_numerical_features.shape[1]
            ...
            for i, game in enumerate(games):
                ...
                vector_today = (
                    np.concatenate([category_vector, scaled_numerical_features[i]]).astype(np.float32).tobytes()
                )

                pipeline.hset(
                    f"game:{game.game_id}",
                    mapping={"title": game.title, "vector_full": vector_full, "vector_today": vector_today},
                )
            ...
            self.create_redis_search_index(
                r, index_name="game_index_full", vector_field_name="vector_full", dim=full_vector_dimension
            )
            self.create_redis_search_index(
                r, index_name="game_index_today", vector_field_name="vector_today", dim=today_vector_dimension
            )
    ...
    def create_redis_search_index(self, r: Redis, index_name: str, vector_field_name: str, dim: int) -> None:
        schema = [
            TextField("title"),
            VectorField(vector_field_name, "HNSW", {"TYPE": "FLOAT32", "DIM": dim, "DISTANCE_METRIC": "COSINE"}),
        ]

        try:
            r.ft(index_name).dropindex(delete_documents=False)
            self.stdout.write(f"기존 인덱스를 교체합니다: {index_name}")
            logger.info(f"기존 검색 인덱스를 교체합니다: {index_name}")
