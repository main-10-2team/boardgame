...
class S3Uploader:
    ...
    def __init__(self) -> None:  # 반환 타입 힌팅 유지
        # AWS 대신 NCP 설정 사용
        ncp_access_key_id = settings.NCP_ACCESS_KEY_ID
        ncp_secret_access_key = settings.NCP_SECRET_ACCESS_KEY
        ncp_region = settings.NCP_REGION
        ncp_endpoint_url = settings.NCP_ENDPOINT_URL  # NCP 엔드포인트 URL

        self.bucket = settings.NCP_STORAGE_BUCKET_NAME
